...
$(document).on('click', "#submitFeedback", function() {
	...
	$.post('https://vrp_ticket/action', JSON.stringify({
		action: "newFeedback",
		subject: subject,
...
function viewFeedbackWindow(feedbackid) {
	...
	feedbackW.find(".player_identifier").html(`${data.idjucator}`)
	...
	if (data.category === 'player_report') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-question"></i> Intrebare</span>`)
	} else if (data.category === 'question') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-users-slash"></i> Reclamatie Player/Staff</span>`)
	} else {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-bug"></i> Report Bug</span>`)
	}
}
...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			category = "<span><i class='fas fa-question'></i> Intrebare</span>"
		} else if (feedback.category === 'question') {
			category = "<span><i class='fas fa-users-slash'></i> Reclamatie Player/Staff</span>"
		} else {
			category = "<span><i class='fas fa-bug'></i> Report Bug</span>"
		}
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					<td>TICKET #${feedback.feedbackid}</th>
					<td>${category}</td>
					<td style="color: ${statusColor};">${concluded}</td>
					<td><button type="button" class="btn btn-view"><i class="fas fa-eye"></i> Vezi</button></td>
				</tr>`
			)
		}
	});
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("Nu exista momentan tickete active");
		...
		$(".feedback_subtitle").html("Lista tickete...");
